  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        return res
          .status(400)
          .json({ error: "sorry a user with this email is already existed" });
      }
...
//  Router 2:authenticate a user using post
router.post(
  "/login",
  [
    body("email", "enter a vaild email").isEmail(),
...
// Route3: get logged in user details using post '/api/auth/getuser. log in required'
